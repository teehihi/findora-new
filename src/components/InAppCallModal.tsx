...
export function InAppCallModal({ visible, otherUser, status = 'RINGING', onClose }: InAppCallModalProps) {
  ...
  // Stream URL Sync Listener
  useEffect(() => {
    let interval: any = null;
    if (visible) {
      interval = setInterval(() => {
        const client = callManager.getWebRTCClient();
        const rStream = client?.RemoteStream;
        const lStream = client?.LocalStream;

        const rUrl = rStream?.toURL ? rStream.toURL() : null;
        const lUrl = lStream?.toURL ? lStream.toURL() : null;

        setRemoteStreamURL((prev) => (prev !== rUrl ? rUrl : prev));
        setLocalStreamURL((prev) => (prev !== lUrl ? lUrl : prev));
      }, 300);
    } else {
  ...
  const handleToggleMic = () => {
    ...
    callManager.setMicrophoneMuted(nextState);
  };

  const handleEndCallAction = () => {
    setCallState('ENDED');
    callManager.endCall(duration).catch(() => {});
    setTimeout(() => {
      onClose(duration);
    }, 600);
  };
  ...
  const getStatusLabel = () => {
    ...
    if (callState === 'CONNECTING' || callState === 'ACCEPTING') return 'Đang kết nối...';
  ...
  const showLocalVideo = isVideoOn && isCallActive;
  const showRemoteVideo = isVideoOn && callState === 'CONNECTED' && !!remoteStreamURL;

  return (
    <Modal visible={visible} animationType="slide" transparent={false}>
      <View style={styles.modalRoot}>
        {/* Fullscreen Remote Video View */}
        {showRemoteVideo && RTCView && remoteStreamURL ? (
          <RTCView
        ...
        <SafeAreaView style={styles.container}>
          {/* Local PIP Video View (Pre-loaded immediately during call startup!) */}
          {showLocalVideo && RTCView && localStreamURL ? (
          ...
          {/* Center Caller Info & Pulsing Avatar (Show when remote video is not connected yet) */}
          {!showRemoteVideo && (
...
const styles = StyleSheet.create({
  ...
  container: {
    flex: 1,
    backgroundColor: 'transparent', // Transparent to let remoteVideo show underneath
